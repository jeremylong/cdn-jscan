...
@Component
@CommandLine.Command(name = "install", description = "Used on mac or unix systems to create the jscan symlink and add completion to the shell.")
public class InstallCommand extends AbstractHelpfulCommand {
    ...
    public Integer call() throws Exception {
        ...
        if (uLocalBin.isDirectory()) {
            link = Paths.get(uLocalBin.getPath(), "jscan");
        } else {
            link = Paths.get(".", "jscan");
        }
        ...
        LOG.info(colorize("jscan link created: " + link.toString(), Attribute.GREEN_TEXT()));

        final Resource completion = resourceLoader.getResource("classpath:jscan.completion.sh");
        if (completion.exists()) {
            ...
            if (zshlinux.isDirectory()) {
                destination = Paths.get(zshlinux.getPath(), "jscan.completion.sh");
            } else if (zshMac.isDirectory()) {
                destination = Paths.get(zshMac.getPath(), "jscan.completion.sh");
            } else {
                destination = Paths.get(".", "jscan.completion.sh");
            }
